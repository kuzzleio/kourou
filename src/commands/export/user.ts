...
export default class ExportUser extends Kommand {
  static description = `Exports users to JSON.
...
`;
  ...
  static flags = {
    ...
    mappings: flags.boolean({
      description: "Export user mappings",
    }),
    ...kuzzleFlags,
  ...
  async beforeConnect() {
    if (
      this.flags["generated-username"] !== "_id" &&
  ...
  async _dumpUsers() {
    ...
    if (this.flags["mappings"]) {
      await this._dumpUserMappings();
    }
