    ...
    const walkObject = (_base: any, _object: any, path: any = []) => {
      ...
      for (const key of Object.keys(_object)) {
        if (_base[key] === undefined) {
          ...
          const currentValue = _.isObject(_object[key]) ? JSON.stringify(_object[key]) : _object[key];
          const previousValue = _.isObject(_base[key]) ? JSON.stringify(_base[key]) : _base[key];
          changes[[...path, key].join('.')] = `value is "${currentValue}" and was "${previousValue}"`
        }
      }
    }
