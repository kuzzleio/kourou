import { useSdk } from "../../helpers/sdk";
    ...
    collection = "yellow-taxi";
      ...
      await sdk.document.create(index, collection, {}, "chuon-chuon-kim");
        ...
        try {
          const { stdout } = await kourou("es:indices:get", "&nyc-open-data.yellow-taxi","chuon-chuon-kim");
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
    ...
    collection = "green-taxi";
        ...
        try {
          const { stdout } = await kourou("es:indices:cat", "--grep","nyc-open-data");
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
    ...
    collection = "green-taxi";
        ...
        try {
          const { stdout } = await kourou("es:indices:insert", "&nyc-open-data.yellow-taxi",["--id","kindred","--body","{}"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
        ...
        try {
          const { stdout } = await kourou("es:indices:get", "&nyc-open-data.yellow-taxi","kindred");
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      it('Create a snapshot repository', async () => {
        ...
        try {
          const { stdout } = await kourou("es:snapshot:create-repository", "backup","/tmp/snapshots",["--compress"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      it('Dump ES data to a snapshot into a repository', async () => {
        ...
        try {
          const { stdout } = await kourou("es:snapshot:create", "backup","test-snapshot",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
    ...
    await sdk.collection.create("nyc-open-data", "yellow-taxi", { mappings: {} });
    index = "nyc-open-data";
    collection = "yellow-taxi";

      await sdk.document.create(index, collection, {"city":"hcmc","district":1}, "chuon-chuon-kim");

      await sdk.document.create(index, collection, {"city":"hcmc","district":2}, "the-hive-vn");

      await sdk.document.create(index, collection, {"city":"changmai","district":7}, "the-hive-th");

    await expect(sdk.document.count(index, collection)).resolves.toBe(3);

        try {
          const { stdout } = await kourou("es:migrate", ["--src","http://localhost:9200","--dest","./kourou-dump"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
    ...
    const lines = fs.readFileSync("./kourou-dump/&nyc-open-data.yellow-taxi.json", "utf8").split("\n");
    expect(lines.filter((line) => line !== "").length).toBe(3);

        try {
          const { stdout } = await kourou("es:migrate", ["--src","./kourou-dump","--dest","http://localhost:9200","--reset","--no-interactive"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
