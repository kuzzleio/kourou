...
describe("InstanceLogs", () => {
  ...
  it("Show logs of specified kuzzle instance", async () => {
    shouldResetSecurity = false;
    ...
    try {
      const { stdout } = await kourou("instance:logs", "--instance", "kuzzle");
      response = stdout;
    } catch (error) {
      console.error(error);
      throw error;
    ...
    expect(response).toMatch(new RegExp("Kuzzle 2.[0-9]+.[0-9]+ is ready"));
  });
});
