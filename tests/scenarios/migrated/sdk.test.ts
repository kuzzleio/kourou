import { useSdk } from "../../helpers/sdk";
    ...
    collection = "yellow-taxi";
        ...
        try {
          const { stdout } = await kourou("api", "document:createOrReplace",["--arg","index=nyc-open-data","--arg","collection=yellow-taxi","-a","_id=chuon-chuon-kim","--body","{ \"other-name\": \"my\" }"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
    ...
    await expect(sdk.document.get(index, collection, "chuon-chuon-kim")).resolves.toMatchObject({ _source: {"other-name":"my"} });
      ...
      it('Impersonate an user', async () => {
        ...
        try {
          const { stdout } = await kourou("api", "auth:getCurrentUser",["--as","gordon"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
    ...
    collection = "yellow-taxi";
        ...
        try {
          const { stdout } = await kourou("execute", "return await sdk.document.create(\"nyc-open-data\", \"yellow-taxi\", {}, \"document-adrien\");",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      await expect(sdk.document.exists(index, collection, "document-adrien")).resolves.toBe(true);
