import { useSdk } from "../../helpers/sdk";
    ...
    collection = "yellow-taxi";
      ...
      response = await sdk.document["mCreate"](
        index,
        collection,
        [{"_id":"chuon-chuon-kim","body":{"city":"hcmc","district":1,"nested":{"field":1}}},{"_id":"the-hive-vn","body":{"city":"hcmc","district":2,"nested":{"field":2}}},{"_id":"the-hive-th","body":{"city":"changmai","district":7,"unwanted":"field"}},{"_id":"luca","body":{"city":"changmai","district":[7,8,9]}},{"_id":"toto","body":{"city":"changmai","district":11,"object":{"field":"this is an object"}}}]
      );
  
    await sdk.collection.refresh(index, collection);
  
        try {
          const { stdout } = await kourou("export:collection", "nyc-open-data","yellow-taxi",["--format","csv","--fields","city,district,nested.field,object"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      expect(fs.readFileSync("nyc-open-data/yellow-taxi/documents.csv").toString()).resolves.toMatchObject("_id,city,district,nested.field,object\nchuon-chuon-kim,hcmc,1,1,\nthe-hive-vn,hcmc,2,2,\nthe-hive-th,changmai,7,,\nluca,changmai,[NOT_SCALAR],,\ntoto,changmai,11,,[NOT_SCALAR]\n");
  });
    ...
    collection = "yellow-taxi";
      ...
      response = await sdk.document["mCreate"](
        index,
        collection,
        [{"_id":"the-hive-th","body":{"city":"changmai","district":7,"unwanted":"field"}},{"_id":"luca","body":{"city":"changmai","district":[7,8,9]}},{"_id":"chuon-chuon-kim","body":{"city":"hcmc","district":1,"nested":{"field":1}}}]
      );
  
    await sdk.collection.refresh(index, collection);
  
        try {
          const { stdout } = await kourou("export:collection", "nyc-open-data","yellow-taxi",["--format","csv"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      expect(fs.readFileSync("nyc-open-data/yellow-taxi/documents.csv").toString()).resolves.toMatchObject("_id,city,firstName,lastName,name\nthe-hive-th,changmai,,,\nluca,changmai,,,\nchuon-chuon-kim,hcmc,,,\n");
  });
      });
