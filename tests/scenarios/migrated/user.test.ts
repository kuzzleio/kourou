import { useSdk } from "../../helpers/sdk";
    ...
    describe("User", () => {
      ...
      it('Export users', async () => {
        ...
        try {
          const { stdout } = await kourou("user:export", ["--path","./dump","--exclude","gordon","--exclude","test-admin"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      fs.writeFileSync(`./dump/user-mapping.json`, {"type":"usersMappings","content":{"mapping":{"profileIds":{"type":"keyword"},"email":{"type":"keyword"},"age":{"type":"integer"}}}});
    
        try {
          const { stdout } = await kourou("user:import-mappings", "./dump/user-mapping.json",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      response = await sdk.query({"controller":"security","action":"getUserMapping"});
    
       expect(response.result["mapping"]).toBeDefined()
  
       expect(response.result["mapping"]).toMatchObject({"profileIds":{"type":"keyword"},"email":{"type":"keyword"},"age":{"type":"integer"}});
    
        try {
          const { stdout } = await kourou("user:export-mappings", ["--path","./dump"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
       ...
       expect(JSON.parse(fs.readFileSync("./dump/users-collection-mappings.json", "utf8"))).toMatchObject({"type":"usersMappings","content":{"mapping":{"age":{"type":"integer"},"email":{"type":"keyword"},"profileIds":{"type":"keyword"}}}});
  });
      });
