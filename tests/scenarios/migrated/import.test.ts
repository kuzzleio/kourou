...
describe("Import", () => {
  ...
  it("Generic import of dump files", async () => {
    ...
    try {
      const { stdout } = await kourou("export:user", [
        "--path",
        "./dump",
        "--exclude",
        "gordon",
    ...
    await expect(
      sdk.document.get(index, collection, "chuon-chuon-kim2")
    ).resolves.toMatchObject({ _source: { city: "hcmc", district: 1 } });

    await expect(
      sdk.document.get(index, collection, "the-hive2")
    ...
    await expect(
      sdk.document.get(index, collection, "the-hive")
    ).resolves.toMatchObject({ _source: { city: "hcmc", district: 2 } });
    ...
    await expect(
      sdk.security.getRole("teacher").content
    ).resolves.toMatchObject({ document: { actions: { create: true } } });

    await expect(
      sdk.security.getRole("student").content
    ).resolves.toMatchObject({ document: { actions: { update: true } } });

    await expect(sdk.security.getProfile("teacher")).resolves.toMatchObject([
      { roleId: "default", restrictedTo: [{ index: "nyc-open-data" }] },
    ]);

    await expect(sdk.security.getProfile("student")).resolves.toMatchObject([
      { roleId: "admin", restrictedTo: [{ index: "mtp-open-data" }] },
    ]);

    await expect(
      sdk.security.getUser("kleiner").content
    ).resolves.toMatchObject({
      profileIds: ["admin"],
      email: "kleiner@blackmesa.us",
    });

    await expect(sdk.security.getUser("alyx").content).resolves.toMatchObject({
      profileIds: ["admin"],
