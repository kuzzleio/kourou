...
describe("Vault", () => {
  ...
  it("Encrypt a secrets file", async () => {
    ...
    try {
      const { stdout } = await kourou("vault:encrypt", "test-secrets.json", [
        "--vault-key",
        "secret-password",
      ]);
      response = stdout;
  ...
  it("Test a secrets file", async () => {
    ...
    try {
      const { stdout } = await kourou("vault:test", "test-secrets.enc.json", [
        "--vault-key",
        "secret-password",
      ]);
      response = stdout;
  ...
  it("Decrypt a secrets file", async () => {
    ...
    try {
      const { stdout } = await kourou(
        "vault:decrypt",
        "test-secrets.enc.json",
        ["--vault-key", "secret-password", "--output-file", "decrypted.json"]
      );
      response = stdout;
    } catch (error) {
      console.error(error);
    ...
    expect(JSON.parse(fs.readFileSync("decrypted.json", "utf8"))).toMatchObject(
      { aws: { s3: "foobar" }, my: { huong: "hcmc" } }
    );
  });
});
