import { useSdk } from "../../helpers/sdk";
    ...
    collection = "yellow-taxi";
      ...
      await sdk.document.create(index, collection, {"name":"Adrien","city":"Saigon"});
    
      await sdk.document.create(index, collection, {"name":"Sebastien","city":"Cassis"});
    
    await sdk.collection.refresh(index, collection);
  
        try {
          const { stdout } = await kourou("document:search", "nyc-open-data","yellow-taxi","{ equals: { city: \"Saigon\" } }",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
