import { useSdk } from "../../helpers/sdk";
    ...
    collection = "yellow-taxi";
        ...
        try {
          const { stdout } = await kourou("document:createOrReplace", ["--arg","index=nyc-open-data","--arg","collection=yellow-taxi","-a","_id=chuon-chuon-kim","--body","{ \"other-name\": \"my\" }"]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
    ...
    await expect(sdk.document.get(index, collection, "chuon-chuon-kim")).resolves.toMatchObject({ _source: {"other-name":"my"} });
  });
    ...
    collection = "yellow-taxi";
      ...
      expect(response).toMatch(new RegExp("yellow-taxi"));
        ...
        try {
          const { stdout } = await kourou("collection:truncate", "nyc-open-data","yellow-taxi",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      expect(response).toMatch(new RegExp("acknowledged"));
        ...
        try {
          const { stdout } = await kourou("document:createOrReplace", "nyc-open-data","yellow-taxi","foobar-1","{helloWorld: 42}",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      expect(response).toMatch(new RegExp("helloWorld"));
        ...
        try {
          const { stdout } = await kourou("document:delete", "nyc-open-data","yellow-taxi","foobar-1",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
      ...
      expect(response).toMatch(new RegExp("foobar-1"));
        ...
        try {
          const { stdout } = await kourou("collection:updateMapping", "nyc-open-data","yellow-taxi","{ dynamic: \"false\" }",[]);
          response = stdout;
        } catch (error) {
          console.error(error);
          throw error;
